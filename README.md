# Freehand

Freehand is a small Rust library that provides some drawing utilities for the [`image`](https://github.com/image-rs/image) crate.

It provides the following:
- circular arcs
- partial annulii (filled circular arcs)
- antialiased circular arcs

## Limitations

Currently only works with the [`RgbaImage`](https://docs.rs/image/latest/image/type.RgbaImage.html) and [`Rgba`](https://docs.rs/image/latest/image/struct.Rgba.html) types



